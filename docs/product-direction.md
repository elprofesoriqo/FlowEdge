...
| Keep | Leave |
|---|---|
| Hardware: CPU now; Jetson/ARM via plugin; CUDA flow and DP heads, then GPU replay ([#163](https://github.com/elprofesoriqo/FlowEdge/issues/163)) | Robot e-stop and joint limits |
...
The sequence is convert → matched CPU replay vs PyTorch → Transformer fixture (#10) → CUDA device-resident flow ([#161](https://github.com/elprofesoriqo/FlowEdge/issues/161)) and DP ([#162](https://github.com/elprofesoriqo/FlowEdge/issues/162)) → a period loop on Jetson or SO-100 with miss counts. Tenstorrent stays behind that GPU evidence. Success is an external team doing that without FlowEdge training code.
