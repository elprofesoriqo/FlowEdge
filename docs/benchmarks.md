...
| SmolVLA | `tools/smolvla_preflight.py`, `verify_smolvla_action_expert.py`, `verify_smolvla_cached_expert.py`, `flowedge-inspect` | Real checkpoint schema, suffix parity, and captured VLM-cache expert replay | preflight + PyTorch max-error + real-capture replay artifact; no native VLM, timing, or control-quality claim |
