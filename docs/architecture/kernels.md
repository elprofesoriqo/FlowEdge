...
workspace through `matmul`), `rmsnorm`, `silu`, `softplus`, `gate_silu`, and
`discretize_and_scan`.
...
backends. Packed Diffusion Policy convolution reuses `matmul`. `rmsnorm` stays
scalar unless a profile says otherwise.
