...
[[nodiscard]] constexpr std::size_t conv_transpose1d_workspace_floats(std::size_t in_channels,
                                                                      std::size_t out_channels,
...
inline void grouped_query_attention(std::span<const float> query, std::span<const float> keys,
                                    ...
                                    std::size_t rows, std::size_t query_heads, std::size_t kv_heads,
                                    std::size_t head_width, std::size_t prefix_size,
                                    bool cross_attention) noexcept
{
    ...
    for (std::size_t head{}; head < query_heads; ++head) {
      ...
      for (std::size_t token{}; token < prefix_size; ++token) {
        ...
        const float* const key =
            (cross_attention ? keys.data() : prefix_keys.data()) +
      ...
      for (std::size_t channel{}; channel < head_width; ++channel) {
        ...
        for (std::size_t token{}; token < prefix_size; ++token) {
          ...
          const float* const value =
              (cross_attention ? values.data() : prefix_values.data()) +
              ((token * kv_heads + kv_head) * head_width);
