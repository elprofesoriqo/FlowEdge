...
#ifdef FLOWEDGE_CUDA
  if (cuda_ != nullptr) {
    if (!ok_ || steps == 0uz || cond.size() != cfg_.cond_dim || x0.size() != cfg_.action_dim ||
        out.size() < cfg_.action_dim)
      return;
                                      ...
                                      std::span<float> out) noexcept
{
  ...
  if (cuda_ != nullptr) {
    ...
    while (completed < todo) {
      ...
      state.next_step += n;
      completed += n;
      copy_span(out.first(cfg_.action_dim), state.x);
    }
